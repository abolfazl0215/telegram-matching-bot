const { computeScore, computeActivity } = require("./computeScore");

// ── FIX #3 (perf): استفاده از baseScore کش‌شده به‌جای محاسبه‌ی زنده ──────────
// قبلاً computeScore(candidate) داخل هر ۴ پاس این تابع صدا زده می‌شد؛ یعنی
// برای هر request، به ازای هر candidate در pool، دوباره از صفر محاسبه می‌شد.
// حالا baseScore در fillPool.js (چرخه‌ی دوره‌ای رفرش pool) و addToPool.js
// (وقتی کاربر تازه وارد pool میشه) از قبل روی خود آبجکت candidate ذخیره شده.
// این تابع فقط baseScore رو می‌خونه؛ اگر به هر دلیلی (مثلاً داده‌ی قدیمی یا
// مسیر غیرمنتظره‌ای که هنوز baseScore رو ست نکرده) این فیلد وجود نداشت،
// به‌عنوان fallback ایمن، همون‌جا محاسبه و روی candidate cache میشه تا
// دفعه‌ی بعد دیگه نیازی به محاسبه‌ی مجدد نباشه.
function getBaseScore(candidate) {
  if (typeof candidate.baseScore === "number") {
    return candidate.baseScore;
  }
  // fallback: نباید عادی اتفاق بیفته، ولی اگه افتاد سرویس نباید بشکنه
  const score = computeScore(candidate);
  candidate.baseScore = score;
  candidate.scoredAt = Date.now();
  return score;
}
...
function getMatchCandidates(viewer, pool, limit = 10) {
  ...
  // FIX #2: تمام منطقی که مستقیماً روی آبجکت‌های مشترک pool/usersMap
  // mutate می‌کنه (candidate.finalScore / candidate._passIndex) داخل
  // try/finally قرار گرفت. اگه وسط حلقه هر exception‌ای رخ بده،
  // finally تضمین می‌کنه فیلدهای موقت از روی آبجکت‌های shared پاک بشن
  // و state کثیف برای viewer/درخواست بعدی باقی نمونه.
  try {
    // ── PASS 1 ────────────────────────────────────────────────────
    for (const candidate of pool) {
      ...
      const base = getBaseScore(candidate);
    ...
    if (scored.length < 50) {
      for (const candidate of pool) {
        ...
        // FIX #1: این چک قبلاً توی Pass 2 جا افتاده بود، در نتیجه
        // کاندیدهای ناسازگار از نظر جنسیت/ترجیح هم وارد فید می‌شدن.
        if (!isCompatible(viewer, candidate)) continue;
    ...
    // نتیجه‌ی نهایی رو نگه می‌داریم تا بعد از finally return بشه
    var _finalResults = results;
    var _resultIds = resultIds;

    return results;
  } finally {
    // ── CLEANUP کامل (تضمین‌شده، حتی روی exception) ────────────────
    // روی کل scored پاک میشه (نه فقط results)، چون همه‌شون مستقیم
    // روی آبجکت‌های واقعی pool/usersMap mutate شدن.
    const resultIds =
      typeof _resultIds !== "undefined" ? _resultIds : new Set();
